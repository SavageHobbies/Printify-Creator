...
function App() {
  ...
  const renderStep = () => {
    switch (currentStep) {
      ...
      case 1:
        return <DesignUpload onUpload={(files) => { setDesignFiles(files); nextStep(); }} />;
      case 2:
        ...
        return (
          <ProductDetails
            onSubmit={(title, description) => {
              ...
              // Here you would integrate with Printify API to create the product listing
              console.log('Product ready for Printify:', { selectedProduct, designFiles, selectedTemplates, productTitle, productDescription });
            }}
          />
        );
      default:
              ...
              {steps.map((step, index) => (
                <React.Fragment key={step}>
                  <div className={`flex items-center ${index <= currentStep ? 'text-blue-600' : 'text-gray-400'}`}>
                    <span className="rounded-full bg-blue-600 text-white px-3 py-1">{index + 1}</span>
